...
class ODF(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_cao = attn * x  # 这里的x_cao确实就是(B, C, H, W)(1, 4, 256, 256)

        # 空间注意力
        x_h = x_cao.mean(dim=3)  # 沿着宽度维度求平均
        l_x_h, g_x_h_s, g_x_h_m, g_x_h_l = torch.split(x_h, self.group_chans, dim=1)  # 拆分通道
        # (B, C, W)
        x_w = x_cao.mean(dim=2)  # 沿着高度维度求平均（1，4, 256）
        l_x_w, g_x_w_s, g_x_w_m, g_x_w_l = torch.split(x_w, self.group_chans, dim=1)  # 拆分通道
        # 这里x_h 和 x_w 确实也是(B, C, H) 和 (B, C, W) (1, 4, 256)

        # ---- 水平注意力 ----

        # 对拆分的4个通道(B, C, H/4) * 4 分别做不同大小卷积核卷积，然后拼接起来(B, C, H)进行softmax
        ...
        # 这个x_h_attn的size也是(B, C, H) (1, 4, 256)

        x_h_attn_ = x_h_attn.view(b, c, h_, 1)  # (B, C, H, 1) 得到sigmoid加权后的(1, 3, 256, 1) 这里

        # 卷积处理（沿 h_ 方向） 使得结果更平滑
        x_h_gate = self.h_gate(x_h_attn_.squeeze(-1))  # (B, C, H) (1, 3, 256)
        x_h_attn_ = x_h_gate.unsqueeze(-1)  # 还原为 (B, C, H, 1) (1, 3, 256, 1)
        ...
        x_w_attn_ = x_w_attn.view(b, c, 1, w_)  # (B, C, 1, W)

        # 卷积处理（沿 w_ 方向）
        x_w_gate = self.w_gate(x_w_attn_.squeeze(2))  # (B, C, W)
        x_w_attn_ = x_w_gate.unsqueeze(2)  # 还原为 (B, C, 1, W)
